...
impl Preprocessor for ContentLoader {
    ...
    fn run(&self, ctx: &PreprocessorContext, mut book: Book) -> Result<Book, Error> {
        ...
        // inject_all = true  # optional, default = false
        let inject_all = match ctx
            .config
            .get::<bool>("preprocessor.content-loader.inject_all")
        {
            Ok(Some(b)) => b,
            Ok(None) => false, // key not set
            Err(e) => {
                log::warn!(
                    "content-loader: expected bool for preprocessor.content-loader.inject_all: {e}"
                );
                false
            }
        };
